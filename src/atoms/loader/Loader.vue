<script setup lang="ts">
interface Props {
  label?: string
}

const { label = 'Initializing' } = defineProps<Props>()
</script>

<template>
  <div class="v-loader">
    <svg
      width="60"
      height="60"
      viewBox="0 0 60 60"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
      xmlns:xlink="http://www.w3.org/1999/xlink"
      class="v-loader__svg"
    >
      <circle cx="30" cy="30" r="30" fill="white" />
      <path
        class="v-loader__arc"
        d="M60 30C60 36.9407 57.5934 43.6666 53.1903 49.0318C48.7872 54.397 42.66 58.0695 35.8527 59.4236C29.0454 60.7776 21.9792 59.7294 15.8581 56.4576C9.73699 53.1858 4.93969 47.8928 2.28361 41.4805C-0.372461 35.0682 -0.722977 27.9333 1.29179 21.2915C3.30656 14.6497 7.56194 8.91194 13.3329 5.05591C19.1038 1.19989 26.0333 -0.535845 32.9405 0.144458C39.8477 0.82476 46.3054 3.87901 51.2132 8.78679L48.3562 11.6438C44.1094 7.39698 38.5215 4.75408 32.5445 4.1654C26.5675 3.57672 20.5713 5.07869 15.5776 8.41538C10.5839 11.7521 6.90163 16.717 5.15821 22.4643C3.41479 28.2116 3.7181 34.3856 6.01646 39.9343C8.31481 45.483 12.466 50.0632 17.7627 52.8943C23.0595 55.7255 29.174 56.6325 35.0645 55.4608C40.955 54.2891 46.2569 51.1112 50.067 46.4686C53.8771 41.826 55.9596 36.0059 55.9596 30L60 30Z"
        fill="url(#paint0_linear_loader)"
      />

      <rect x="13" y="13" width="34" height="34" fill="url(#pattern0_loader)" />

      <defs>
        <pattern id="pattern0_loader" patternContentUnits="objectBoundingBox" width="1" height="1">
          <use xlink:href="#image0_loader" transform="scale(0.00444444)" />
        </pattern>
        <linearGradient
          id="paint0_linear_loader"
          x1="60"
          y1="30"
          x2="0"
          y2="30"
          gradientUnits="userSpaceOnUse"
        >
          <stop stop-color="#8743FF" />
          <stop offset="1" stop-color="#D8C2FF" />
        </linearGradient>

        <image
          id="image0_loader"
          width="225"
          height="225"
          preserveAspectRatio="none"
          xlink:href="data:image/png;base64,"
        />
      </defs>
    </svg>
    <span class="v-loader__label">{{ label }}</span>
  </div>
</template>

<style lang="scss" scoped>
.v-loader {
  display: flex;
  flex-direction: column;
  gap: 10px;
  align-items: center;
  justify-content: center;
  background: transparent;

  &__svg {
    width: 60px;
    height: 60px;
  }

  &__arc {
    transform-origin: center;
    animation: spin 1.5s linear infinite;
  }

  &__label {
    font-family: 'Poppins', sans-serif;
    font-size: 14px;
    line-height: 21px;
    text-align: center;
    white-space: nowrap;
    color: #ffffff;
  }
}

@keyframes spin {
  from {
    transform: rotate(0deg);
  }
  to {
    transform: rotate(360deg);
  }
}
</style>
